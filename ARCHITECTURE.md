...
└──────────┬──────────┘
           │  blaze generate  (or blaze dev, which generates on start)
           ▼
┌──────────────────────┐
...
The `SchemaLoader` (`packages/schema/src/loader.ts`) resolves the raw user files, the validator checks them, and the watcher enables hot reload during `blaze dev`.
...
`blaze deploy` builds the admin SPA and runs `firebase deploy --only hosting`. Firestore rules, storage rules, and indexes (generated by `blaze generate`) are deployed with the Firebase CLI against the target project (e.g. the playground uses the `arche-cms-demo` project; keys live in the project `.env` as `VITE_*` client vars).
